...
const logRequest = (config) => {
  ...
  console.log(
    `[API_REQ][${timestamp}] ${config.method.toUpperCase()} ${config.url}`
  );
};
...
api.interceptors.request.use(
  (config) => {
    ...
    if (config.method.toLowerCase() === 'get') {
      ...
      // If a request with the same key is already pending,
      // abort the old one and let the new one proceed.
      if (pendingRequests.has(requestKey)) {
        console.warn(
          `[API_PREV_ABORT] Aborting stale request: ${config.url}`
        );
  ...
  async (error) => {
    ...
    if (error.response?.status === 429) {
      ...
      if (retryCount < MAX_RETRIES) {
        ...
        console.warn(
          `[API_429] Rate limited. Retrying ${config.url} in ${Math.round(
            delay
          )}ms... (Attempt ${config.__retryCount}/${MAX_RETRIES})`
        );
      ...
      console.error(
        `[API_429_MAX] Max retries reached for ${config.url}`
      );
